...
def Label(images: np.ndarray, edges: np.ndarray, foregroundThreshold: float,
          gaussianSigma: float):
    print("Labeling objects...", end="", flush=True)
    labeledImages = np.zeros(images.shape, np.uint32)
    for i in range(images.shape[0]):
        ...
        labeledImages[i] = labeled + unsplit_labeled
    printRep("Done.")
    printRep(None)

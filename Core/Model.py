...
def TrainModel(model: tf.keras.Model, learningRate, patience, epochs, batchSize,
    ...
    model.fit(x=ImageGenerator(trainingData, batchSize, inputSize),
...
def LoadGroundTruths(groundTruths: List[GroundTruth], model: ModelType):
    size = InputSize(model)
    images = PrepareImagesForModel([Image.open(x.imagePath) for x in groundTruths], size)
    segmentations = PrepareSegmentationsForModel(
        [Image.open(x.segmentationPath) for x in groundTruths], size)
...
def ComputeIOUs(model: Union[tf.keras.Model, tf.lite.Interpreter], images: List[Image.Image],
    ...
    outputImages = Detect(model, PrepareImagesForModel(images, model)) >= 0.5

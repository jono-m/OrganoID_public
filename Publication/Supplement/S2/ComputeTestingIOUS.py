...
liteModel = LoadLiteModel(Path(r"Publication\Model\model.tflite"))

iousFile = open(Path(r"Publication\Supplement\S2\IOUs.csv"), mode="w+")
for dataset, name in zip(datasets, names):
    images = LoadPILImages(Path(r"Publication\Dataset\testing\images\%s*" % dataset))
    segmentations = LoadPILImages(Path(r"Publication\Dataset\testing\segmentations\%s*" % dataset))

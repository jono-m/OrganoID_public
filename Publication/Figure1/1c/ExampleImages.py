...
images = PrepareImagesForModel(LoadPILImages(
    [Path(r"Publication\Dataset\testing\images\%s*" % name) for name in exampleImageNames]), model)
segmentations = PrepareSegmentationsForModel(LoadPILImages(
    [Path(r"Publication\Dataset\testing\segmentations\%s*" % name) for name in exampleImageNames]),
    model)

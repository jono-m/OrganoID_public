...
pilImages = LoadPILImages(Path(r"Publication\Dataset\testing\images"))
images = PrepareImagesForModel(pilImages, model)
segmentations = PrepareSegmentationsForModel(
    LoadPILImages(Path(r"Publication\Dataset\testing\segmentations")), model)

detections = Detect(model, images)
organoID_labeled = Cleanup(Label(detections, DetectEdges(detections, 2, 0.005, 0.05, 0.5), 0.5, 2),
                           200, False, False)
